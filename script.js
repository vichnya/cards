...
function renderFolders(){
    ...
    Object.keys(users[currentUser].folders).forEach(name => {
        ...
        menuBtn.onclick = e => {
            e.stopPropagation();
            const menu = li.querySelector(".folder-menu");
...
function deleteFolder(name){ 
    if(confirm("Удалить папку "+name+"?")){ 
        ...
        save(); 
        renderFolders(); 
        if(currentFolder===name){cardsGrid.innerHTML=""; currentFolderTitle.textContent="";} 
    } 
}
...
function swapFolders(a,b){ 
    ...
    users[currentUser].folders=newObj; 
    save(); 
    renderFolders(); 
}
...
function openFolder(name){ 
    ...
    editMode=false; deleteMode=false; selectedCards.clear();
    renderCards(); 
    hideCardActionsMenu();
}
...
function createCardActionsMenu(){
    ...
    menu.innerHTML=`<button onclick="enterEditMode()">Редактировать карточку</button>
                    <button onclick="enterDeleteMode()">Удалить</button>`;
    document.querySelector(".folder-header").appendChild(menu);
}

function toggleCardActionsMenu(){
    const menu = document.getElementById("cardActionsMenu");
    menu.style.display = menu.style.display === "flex" ? "none" : "flex";
}
function hideCardActionsMenu(){ document.getElementById("cardActionsMenu").style.display="none"; }

// ===== MODE HANDLERS =====
function enterEditMode(){ 
    editMode=true; deleteMode=false; selectedCards.clear(); renderCards(); showModeExitButton(); hideCardActionsMenu();
}
function enterDeleteMode(){ 
    deleteMode=true; editMode=false; selectedCards.clear(); renderCards(); showModeExitButton(); hideCardActionsMenu();
}

function showModeExitButton(){
    currentFolderTitle.innerHTML = currentFolder + ` <button id="modeExitBtn" onclick="exitMode()">✖</button>`;
}
function exitMode(){
    editMode=false; deleteMode=false; selectedCards.clear();
    renderCards();
...
function updateDeleteExitButton(){
    const btn = document.getElementById("modeExitBtn");
    if(selectedCards.size>0){
        btn.textContent="🗑";
        btn.onclick=confirmDeleteSelectedCards;
    } else {
        btn.textContent="✖";
        btn.onclick=exitMode;
    }
}
    ...
    users[currentUser].folders[currentFolder].forEach((c,i)=>{
        ...
        div.onclick = e=>{
            if(deleteMode){
                ...
                updateDeleteExitButton();
            } else if(editMode){
                const q=prompt("Вопрос:",c.q);
                const a=prompt("Ответ:",c.a);
                if(q!=null && a!=null){
                    users[currentUser].folders[currentFolder][i]={q,a};
                    save(); renderCards();
                }
            } else {
                openFullscreen(i);
...
function openFullscreen(index){ 
    currentCardIndex=index; flipped=false; fullscreen.style.display="flex"; renderFullscreen(); 
}
function closeFullscreen(){ fullscreen.style.display="none"; }
function renderFullscreen(){ 
    const c=users[currentUser].folders[currentFolder][currentCardIndex]; 
